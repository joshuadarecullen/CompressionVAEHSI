...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.conv8 = nn.ConvTranspose2d(32, 28, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, z):
        ...
        z = self.conv8(z)
        reconstructed, uncertainty = torch.split(z, 14, dim=1)
        reconstructed = torch.sigmoid(reconstructed)  # Using sigmoid for final layer to normalize the output
        uncertainty = torch.exp(uncertainty) # ensure positivity
